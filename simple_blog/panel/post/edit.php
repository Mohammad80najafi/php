...
$query = 'SELECT * FROM simple_blog.posts WHERE id = ?';
...
if (
    ...
    $query = 'SELECT * FROM simple_blog.categories WHERE id = ?';
    $statement = $pdo->prepare($query);
    ...
    if (isset($_FILES['image']) && $_FILES['image']['name'] !== '') {
        ...
        if ($category !== false && $image_upload !== false) {
            $query = 'UPDATE  simple_blog.posts SET title = ?, cat_id = ?, body = ?, image = ?, updated_at = NOW() WHERE id = ? ;';
            $statement = $pdo->prepare($query);
            $statement->execute([$_POST['title'], $_POST['cat_id'], $_POST['body'], $image, $_GET['post_id']]);
        }
    } else {
        if ($category !== false) {
            $query = 'UPDATE  simple_blog.posts SET title = ?, cat_id = ?, body = ?, updated_at = NOW() WHERE id = ? ;';
            $statement = $pdo->prepare($query);
                    ...
                    <form action="<?= url('panel/post/edit.php?post_id=' . $_GET['post_id']) ?>" method="post" enctype="multipart/form-data">
                        ...
                        <section class="form-group">
                            ...
                            <select class="form-control" name="cat_id" id="cat_id">
                                <?php
                                global $pdo;
                                $query = "SELECT * FROM simple_blog.categories";
                                $statement = $pdo->prepare($query);
                                $statement->execute();
